...
const Topbar: React.FC<{ breadcrumb?: string; title?: string }> = ({ breadcrumb, title }) => {
  const { user } = useAuth();
    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-neutral-200">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
        <div className="leading-tight">
          {breadcrumb && <p className="text-[11px] text-neutral-500 mb-1">{breadcrumb}</p>}
          {title && <h1 className="text-xm font-semibold text-orange-950">{title}</h1>}
        </div>
